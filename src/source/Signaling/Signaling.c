        ...
        PSignalingClient *ppSignalingClient)
{
    ...
    if (pSignalingClient->pChannelInfo->cachingPolicy == SIGNALING_API_CALL_CACHE_TYPE_FILE) {
        CHK_STATUS(signalingCacheLoadFromFile(pChannelInfo->pChannelName, pChannelInfo->pRegion, pChannelInfo->channelRoleType, &fileCacheEntry, &cacheFound));
        if (cacheFound) {
        ...
        if (apiCall) {
            ...
            if (STATUS_SUCCEEDED(retStatus)) {
                ...
                if (STATUS_SUCCEEDED(retStatus)) {
                    ...
                    if (pSignalingClient->pChannelInfo->cachingPolicy == SIGNALING_API_CALL_CACHE_TYPE_FILE) {
                        signalingFileCacheEntry.creationTsEpochSeconds = GETTIME() / HUNDREDS_OF_NANOS_IN_A_SECOND;
                        ...
                        CHK_STATUS(signalingCacheSaveToFile(&signalingFileCacheEntry));
                    }
                }
            }
